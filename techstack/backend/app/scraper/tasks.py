...
def poll_stories(self, user_id: Optional[str] = None):
    ...
    Pipeline:
    1. Load Instagram session from database
    2. Fetch active stories via API
    ...
    # For V-1, we use settings credentials directly.
    # In production, this would load per-user sessions from the DB.
    ...
    with tempfile.TemporaryDirectory(prefix="memwault_") as tmp_dir:
        ...
        for story_data in stories:
            ...
            # TODO: Check database to skip already-processed stories
            # For now, process all
            ...
            try:
                ...
                # Step 2: Fetch viewers (while story is still active)
                try:
                    viewers = scraper.fetch_story_viewers(media_id)
                    story_data["viewers"] = viewers
                    story_data["viewer_count"] = len(viewers)
                except Exception as ve:
                    logger.warning("Could not fetch viewers for %s: %s", media_id, ve)

                # Step 3: Write metadata into file headers
                ...
                story_data["s3_key_compressed"] = s3_key
                story_data["file_name"] = file_path.name
                story_data["is_downloaded"] = True
                story_data["is_metadata_written"] = True
                story_data["is_uploaded_to_s3"] = True
